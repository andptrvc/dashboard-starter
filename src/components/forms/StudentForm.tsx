...
import { studentSchema, StudentSchemaFormType } from "@/lib/formValidationSchemas";
      ...
      <div className="flex justify-between flex-wrap gap-2">
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          <select
            ...
            {classes.map((c: { id: string; name: string; capacity: number; _count: { students: number } }) => (
              <option
                value={c.id}
                key={c.id}>
          ...
          {errors.classId?.message && <p className="text-sm text-red-400">{errors.classId?.message.toString()}</p>}
        </div>
      </div>
